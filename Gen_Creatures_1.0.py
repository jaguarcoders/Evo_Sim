                ...
                for i in range(len(bodypart_pos[limb_stepper])):
                    if appendage_stepper % 2 == 0 or appendage_stepper == 0:
                        ...
                        for i in range(len(bodypart_pos[limb_stepper][appendage_stepper])):
                            ...
                            window.create_line(appendage_x_anchor,appendage_y_anchor,phalange_x_anchor,phalange_y_anchor,width= width[2], fill=  '#%02x%02x%02x' % ((0, 0, bodypart_str[limb_stepper][appendage_stepper][phalange_stepper] + 155)))
                            window.create_oval(phalange_x_anchor + (width[2]/2) - 1, phalange_y_anchor + (width[2]/2) - 1, phalange_x_anchor - (width[2]/2) + 1, phalange_y_anchor - (width[2]/2) + 1, fill= appendage_color, outline= appendage_color)
                            phalange_stepper += 1
                        phalange_stepper = 0
                        window.create_oval([appendage_x_anchor + (width[1]/2)], [appendage_y_anchor + (width[1]/2)], [appendage_x_anchor - (width[1]/2)], [appendage_y_anchor - (width[1]/2)], fill= limb_color, outline= limb_color)
                    appendage_stepper += 1
                appendage_stepper = 0
...
def create_creatures(creature_number, min_limb_str, max_limb_str, max_limb, min_appendage_str, max_appendage_str,max_appendage, min_phalange_str, max_phalange_str, max_phalange):
    window = Canvas(Tk(), width= 1000, height= 700,background= "black")
    window.pack()
    creature_id = 0
    mover = 100
    body_x_anchor = -50
    ...
    for i in range(creature_number):
        ...
        if body_x_anchor < 1000 - mover:    # Changes x movement
            body_x_anchor += mover
